// Main constants from React and Recharts libraries
...
useEffect(() => {
  ...
  const prepareChartData = () => {
    if (!wardStats || Object.keys(wardStats).length === 0) return [];
    
    return wardOrder.map(ward => {
      if (!wardStats[ward] || !wardStats[ward][selectedMetric]) {
        return { ward };
      }
      
      const stat = wardStats[ward][selectedMetric];
  ...
  const createSummaryTableData = () => {
    ...
    return metrics.map(metric => {
      ...
      wardOrder.forEach(ward => {
        if (wardStats[ward] && wardStats[ward][metric]) {
          const stat = wardStats[ward][metric];
  ...
  if (loading) {
    return <div className="p-4">Loading patient flow data...</div>;
  }
